...
Water::Water()
{
	...
	tmp_case_offset_ = case_offset_ = D3DXVECTOR2( 0, 0 );
}
	...
	float				zoom)
{
	...
	D3DXVECTOR2 case_offset = case_offset_ + tmp_case_offset_;
	float k = DISPLAY_MAG / INITIAL_DISTANCE;
	float minx = 0 + case_offset.x;
	float miny = 0 + case_offset.y;
	float maxx = ( BOX_WIDTH + INITIAL_DISTANCE ) * k + case_offset.x;
	float maxy = ( BOX_HEIGHT + INITIAL_DISTANCE ) * k + case_offset.y;

	vertices[0].p = r.v4( r.mv + D3DXVECTOR2( minx, miny ) );
	vertices[1].p = r.v4( r.mv + D3DXVECTOR2( minx, maxy ) );
	vertices[2].p = r.v4( r.mv + D3DXVECTOR2( minx, maxy ) );
	vertices[3].p = r.v4( r.mv + D3DXVECTOR2( maxx, maxy ) );
	vertices[4].p = r.v4( r.mv + D3DXVECTOR2( maxx, maxy ) );
	vertices[5].p = r.v4( r.mv + D3DXVECTOR2( maxx, miny ) );

	for( int i = 0 ; i < 6 ; i++ ) {
		vertices[i].c = D3DCOLOR_ARGB( 255, 255, 255, 255 );
	}
...
//****************************************************************
// set_case_offset
void Water::set_case_offset( const D3DXVECTOR2& o )
{
	case_offset_ = o;
#if 0
	sph_.set_case_offset(
		( case_offset_ + tmp_case_offset_ ) /
		( DISPLAY_MAG / INITIAL_DISTANCE ) );
#endif
}

//****************************************************************
// set_tmp_case_offset
void		Water::set_tmp_case_offset( const D3DXVECTOR2& o )
{
	tmp_case_offset_ = o;
#if 0
	sph_.set_case_offset(
...
//****************************************************************
// get_case_offset
D3DXVECTOR2 Water::get_case_offset()
{
	return case_offset_;
}
